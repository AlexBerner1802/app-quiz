...
const ModulesManager = ({ modules: initialModules, loading, showLoader, canManage = false }) => {
						...
						{langs.map((lang) => (
							<LanguageBlock key={lang}>
								...
								{canManage ? (
									...
									<ReadOnlyTags>
										{(tempModules[lang] || []).length > 0 ? (
											(tempModules[lang] || []).map((module) => (
												<Tag key={`${lang}-${module.id ?? module.name}`} size="s" variant="secondary">
													{module.name}
												</Tag>
											))
										) : (

...
export default function NewQuiz() {
	...
	const buildMultiPayload = () => {
		...
		const hasMods =
			Array.isArray(d.selectedModuleIds) &&
			d.selectedModuleIds.length > 0;
		const hasTags =
			Array.isArray(d.selectedTagIds) &&
			d.selectedTagIds.length > 0;
		return (
			d.hasTranslation ||
			hasText ||
			hasQs ||
		...
		const languages = LANGS.filter((l) =>
		isMeaningful(drafts[l.code])
		).map((l) => {
	...
	const onSave = async ({ navigateAfter = false } = {}) => {
		...
		console.log("[QUIZ PAYLOAD]", {
			modulesByLang: payload.languages.map((l) => ({
			code: l.code,
			module_ids: l.module_ids,
			})),
			tagsByLang: payload.languages.map((l) => ({
			code: l.code,
			tag_ids: l.tag_ids,
			})),
		});
		...
		if (res?.data?.mapping) {
			...
			quizId ??
			null;
			const safeId = newQuizId != null ? String(newQuizId) : null;
			const qIdByIndex = {};
			const aIdByIndex = {};
			...
			LANGS.forEach((l) => {
				...
				isDirty: false,
				hasTranslation: true,
				questions: (next[l.code].questions || []).map(
					(q, idxQ) => {
					const qMap = qIdByIndex[idxQ];
					const answerIds = (q.options || []).map(
						(_, idxA) =>
						aIdByIndex[idxQ]?.[idxA]?.id ??
						q.answerIds?.[idxA] ??
						null
					);
					return {
			...
			if (!isEdit && safeId) {
			safeNavigateToEditor(navigate, quiz?.id);
			}
		} else {
			setDrafts((prev) => {
			const next = { ...prev };
	...
	const onDeleteLang = (langCode) => {
		const otherLangs = Object.entries(drafts).filter(
		([code]) => code !== langCode
		);
		...
		const fallback =
			Object.keys(drafts).find((c) => c !== langCode) ||
			i18n.language;
	...
	return (
		<>
		<FaviconTitle
			title={t("pages.createPage")}
			iconHref={faviconUrl}
		/>

		<Main>
			<UnsavedChangesGuard
				when={draft.isDirty || hasOtherDirty}
			/>
